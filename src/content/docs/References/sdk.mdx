...
The Magi SDK is a library that abstract away various functionalities that are useful in the context of writing a Magi smart contract.

<Aside type="tip" title="Nice To Know">
They also serve access to the smart contract's database layer by exposing namespaces and interfaces. At the stage of the contract execution, invocations of such interfaces are translated to generalized calls of the Magi node. Thereby ensuring that those functionalities are equal regardless of the language implementation of the SDK.
</Aside>

## Go Contract SDK

The [Go contract SDK](https://github.com/vsc-eco/go-contract-template) implements the necessary runtime and SDK functions for the Magi smart contract environment.
...
Retrieve the balance of any Magi account or contract.
...
Returns `created` upon successful request, or `already_exists` if the key already exists.

```go
//go:wasmexport tssCreateKey
func TssCreateKey(a *string) *string {
  // ...
  status := sdk.TssCreateKey("key id", "ecdsa")
  // ...
}
```
...
Status may be `null`, `created`, `active` or `deleted`.

```go
//go:wasmexport tssGetKey
func TssGetKey(a *string) *string {
  // ...
  key := sdk.TssGetKey("key id")
  // ...
}
```
...
Request a digest to be signed by the TSS key. The key ID must exist and in `active` status.

The string returned may be `ok` or `fail`.

```go
//go:wasmexport tssSignKey
func TssSignKey(a *string) *string {
  // ...
  digest := []byte{} // must be 32-bytes in length
  status := sdk.TssSignKey("key id", digest)
  // ...
}
```
...
|`block.id`|L1 block ID in which the transaction is included in. For offchain transactions, this refers to the L1 block ID of the Magi block in which the transaction was included in.|
|`block.height`|L1 block number in which the transaction is included in. For offchain transactions, this refers to the L1 block number of the Magi block in which the transaction was included in.|
...
Executes a contract call transaction. Returns the call result consisting execution status, error symbol and message (if any), return value, RC used, WASM gas used, logs emitted and state diff summary.
...
func TestContract(t *testing.T) {
  ...
  assert.LessOrEqual(t, result.RcUsed, uint(100))
  assert.LessOrEqual(t, result.GasUsed, uint(10000000))

  // at least one log emitted
  assert.GreaterOrEqual(t, len(result.Logs[contractId].Logs), 1)

  // value of someKey changed from oldvalue to newvalue
  assert.Equal(t, "oldvalue", string(result.StateDiff[contractId].KeyDiff["someKey"].Previous))
  assert.Equal(t, "newvalue", string(result.StateDiff[contractId].KeyDiff["someKey"].Current))

  // deleted value from deletethis key
  assert.True(t, result.StateDiff[contractId].Deletions["deletethis"])
  // ...
}
```
