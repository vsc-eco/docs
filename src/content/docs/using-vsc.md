...
*Introduction to concepts that help make sense of services Magi is able to provide.*

## Automated Market Makers (AMMs) & Liquidity Pools

**Automated Market Makers (AMMs)** are a key component of Magi's decentralized finance (DeFi) ecosystem. AMMs allow users to trade assets automatically through liquidity pools, without the need for centralized order books or market makers. This system provides liquidity in the Magi protocol, allowing for asset swaps across chains.
...
**HBD**, Magi’s stablecoin, serves as the **base asset** in all liquidity pools. This means that every liquidity pair involves **HBD** as one of the assets.
...
Magi uses an **Automated Market Maker (AMM)** algorithm to maintain the balance of assets in the liquidity pool. When a user performs a trade (e.g., swapping BTC for HBD or vice versa), the AMM adjusts the ratio of assets in the pool automatically.
...
LPs can earn rewards continuously, making liquidity provision a passive income opportunity within Magi’s ecosystem.

## Running a Node & Staking Hive as Collateral

To participate in Magi's validation and security mechanisms, users can choose to run a **validator node** or stake Hive as collateral.

- **Running a Node:** To become a validator in Magi’s decentralized network, users need to run a node that actively participates in verifying transactions and securing assets locked on the platform. Running a node contributes to Magi's decentralized validation system and ensures the smooth functioning of the protocol.
...
- **Interacting with Magi (ALTERA):** Altera is Magi's first dApp, a platform for managing assets and cross-chain swaps within Magi. It enables users to handle asset management, execute cross-chain swaps, and interact with the Magi ecosystem. Altera supports transactions and asset tracking across multiple blockchains, providing a unified interface that simplifies decentralized finance (DeFi) activities without requiring users to manage the technical details of each individual blockchain.